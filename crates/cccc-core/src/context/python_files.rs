...
use super::python_storage::PythonContextPaths;
...
fn write_task(paths: &PythonContextPaths, task: &Map<String, Value>) -> io::Result<()> {
    let id = task
        .get("id")
        .and_then(Value::as_str)
        .filter(|id| is_task_id(id))
        .ok_or_else(|| io::Error::other("task id must match Python T<number> format"))?;
