...
fn live_launch_config(root: &std::path::Path) -> LaunchConfig {
    ...
    config.runtime = ActorRuntime::Grok;
    config.command = vec![
...
#[tokio::test]
async fn live_grok_managed_session_when_explicitly_enabled() {
    ...
    std::fs::write(root.join("managed-marker.txt"), "TOPAZ_GROK_MANAGED\n").expect("fixture");

    let session = AnalystSession::launch(&home, live_launch_config(&root))
        .await
        .expect("launch live Grok Analyst");
    ...
    assert!(
        session
            .tui_command()
            .windows(2)
            .any(|parts| parts == ["--resume", session_id.as_str()])
    );
    ...
    let turn = session
        ...
        .await
        .expect("start live Grok turn");
    assert!(
        wait_for_turn_text(&mut events, &turn.turn_id)
            .await
            .contains("TOPAZ_GROK_MANAGED")
    );
    ...
    let cancelled = session
        .start_turn(
            &generation,
            "grok-live-cancel",
            "Use the shell tool to run exactly `printf READY > cancel-ready.txt; sleep 60`, then reply with LATE_GROK_RESULT.",
        )
        .await
    ...
    .await
    .expect("Grok cancellation fixture did not enter shell execution");
    session
        .interrupt(&generation, &cancelled.turn_id)
        .await
        .expect("cancel live Grok turn");
    assert_eq!(
        wait_for_turn_status(&mut events, &cancelled.turn_id).await,
        "cancelled"
    );
    session.stop(&generation).await.expect("stop live Grok");
    ...
    let resumed = AnalystSession::launch(&home, resume)
        .await
        .expect("resume live Grok Analyst");
    ...
    let continuity = resumed
        ...
        .await
        .expect("start resumed Grok turn");
    assert!(
