...
mod tests {
    use super::{
        actor_setting_with_process, browser_delivery_requested, clear_warmup_attempt,
        group_state_allows_delivery, warmup_due,
    };
    use cccc_contracts::{Actor, GroupState};

    #[test]
    fn explicit_pull_mode_wins_over_a_browser_provider() {
        let mut actor = Actor::new("web1");
        actor
            .env
            .insert("CCCC_WEB_MODEL_DELIVERY_MODE".into(), "pull".into());
        assert!(!browser_delivery_requested(&actor, "chatgpt_web"));

        actor
            .env
            .insert("CCCC_WEB_MODEL_DELIVERY_MODE".into(), "browser".into());
        assert!(browser_delivery_requested(&actor, ""));
    }

    #[test]
    fn browser_providers_enable_default_browser_delivery() {
        let actor = Actor::new("web1");
        assert!(browser_delivery_requested(&actor, "chatgpt_web"));
        assert!(browser_delivery_requested(&actor, "chatgpt"));
        assert!(!browser_delivery_requested(&actor, ""));
    }

    #[test]
    fn process_setting_is_used_only_when_actor_setting_is_absent() {
        let mut actor = Actor::new("web1");
        let names = ["CCCC_WEB_MODEL_DELIVERY_MODE", "CCCC_WEB_MODEL_DELIVERY"];
        let process =
            |name: &str| (name == "CCCC_WEB_MODEL_DELIVERY_MODE").then(|| " Browser ".to_owned());
        assert_eq!(
            actor_setting_with_process(&actor, &names, process),
            "browser"
        );
        ...
        actor
            ...
            .insert("CCCC_WEB_MODEL_DELIVERY_MODE".into(), " pull ".into());
        assert_eq!(
            actor_setting_with_process(&actor, &names, |_| Some("browser".into())),
            "pull"
        );

        actor
            .env
            .insert("CCCC_WEB_MODEL_DELIVERY_MODE".into(), " ".into());
        actor.env.insert(
            "CCCC_WEB_MODEL_DELIVERY".into(),
            " browser_delivery ".into(),
        );
        assert_eq!(
            actor_setting_with_process(&actor, &names, |_| Some("pull".into())),
            "browser_delivery"
        );
    }

    #[test]
    fn paused_and_stopped_groups_disable_browser_delivery() {
        assert!(group_state_allows_delivery(true, GroupState::Active));
        assert!(group_state_allows_delivery(true, GroupState::Idle));
        assert!(!group_state_allows_delivery(true, GroupState::Paused));
        assert!(!group_state_allows_delivery(true, GroupState::Stopped));
        assert!(!group_state_allows_delivery(false, GroupState::Active));
    }

    #[test]
    fn failed_warmup_attempts_are_cooled_down_until_cleared() {
        let key = "web-model::test-warmup::actor";
        clear_warmup_attempt(key);
        assert!(warmup_due(key));
        assert!(!warmup_due(key));
        clear_warmup_attempt(key);
        assert!(warmup_due(key));
        clear_warmup_attempt(key);
