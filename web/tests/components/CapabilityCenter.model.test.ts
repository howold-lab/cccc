...
function row(
  overrides: Partial<CapabilityOverviewItem> & { capability_id: string },
...
describe("CapabilityCenter model", () => {
  it("normalizes skills, MCP toolpacks, and packs into first-class center types", () => {
    expect(capabilityCenterType(row({ capability_id: "skill:team:review", kind: "skill" }))).toBe(
      "skill",
    );
    expect(capabilityCenterType(row({ capability_id: "mcp:context7", kind: "mcp_toolpack" }))).toBe(
      "mcp",
    );
  ...
  it("uses the capability id tail as a stable display fallback", () => {
    expect(
      capabilityCenterDisplayName(row({ capability_id: "skill:team:requesting-code-review" })),
    ).toBe("requesting-code-review");
    expect(
      capabilityCenterDisplayName(row({ capability_id: "skill:team:review", name: "Reviewer" })),
    ).toBe("Reviewer");
  });
  ...
  it("filters dense rows by type, query, enabled state, slash visibility, and health", () => {
    const items = [
      ...
      row({
        ...
        kind: "mcp_toolpack",
        tool_names: ["resolve-library-id"],
      }),
      row({ capability_id: "pack:blocked", kind: "pack", blocked_global: true }),
      row({
        capability_id: "skill:team:needs-env",
        readiness_preview: { preview_status: "missing_env" },
      }),
    ];

    expect(
      filterCapabilityCenterItems(items, { typeFilter: "mcp", state }).map(
        (item) => item.capability_id,
      ),
    ).toEqual(["mcp:context7"]);
    expect(
      filterCapabilityCenterItems(items, { query: "resolve-library", state }).map(
        (item) => item.capability_id,
      ),
    ).toEqual(["mcp:context7"]);
    expect(
      filterCapabilityCenterItems(items, { stateFilter: "enabled", state }).map(
        (item) => item.capability_id,
      ),
    ).toEqual(["skill:team:review", "skill:team:hidden", "mcp:context7"]);
    expect(
      filterCapabilityCenterItems(items, { stateFilter: "slash_visible", state }).map(
        (item) => item.capability_id,
      ),
    ).toEqual(["skill:team:review"]);
    expect(
      filterCapabilityCenterItems(items, { stateFilter: "slash_hidden", state }).map(
        (item) => item.capability_id,
      ),
    ).toEqual(["skill:team:hidden"]);
    expect(
      filterCapabilityCenterItems(items, { stateFilter: "blocked", state }).map(
        (item) => item.capability_id,
      ),
    ).toEqual(["pack:blocked"]);
    expect(
      filterCapabilityCenterItems(items, { stateFilter: "needs_setup", state }).map(
        (item) => item.capability_id,
      ),
    ).toEqual(["skill:team:needs-env"]);
  });
  ...
  it("normalizes pagination and reports the visible range", () => {
    expect(
      normalizeCapabilityCenterPagination({ pageIndex: -2, pageSize: 0, totalCount: 161 }),
    ).toEqual({ pageIndex: 0, pageSize: 80, totalPages: 3, offset: 0 });
    expect(
      normalizeCapabilityCenterPagination({ pageIndex: 9, pageSize: 40, totalCount: 161 }),
    ).toEqual({ pageIndex: 4, pageSize: 40, totalPages: 5, offset: 160 });
    expect(
      capabilityCenterPageRange({ pageIndex: 1, pageSize: 40, itemCount: 40, totalCount: 161 }),
    ).toEqual({ from: 41, to: 80, total: 161 });
    expect(
      capabilityCenterPageRange({ pageIndex: 4, pageSize: 40, itemCount: 1, totalCount: 161 }),
    ).toEqual({ from: 161, to: 161, total: 161 });
  });

  it("uses client pagination for state filters that cannot be represented by overview totals", () => {
    expect(capabilityCenterPaginationMode({ section: "skill", stateFilter: "enabled" })).toBe(
      "client",
    );
    expect(capabilityCenterPaginationMode({ section: "mcp", stateFilter: "enabled" })).toBe(
      "client",
    );
    expect(capabilityCenterPaginationMode({ section: "skill", stateFilter: "slash_hidden" })).toBe(
      "client",
    );
    expect(capabilityCenterPaginationMode({ section: "skill", stateFilter: "needs_setup" })).toBe(
      "client",
    );
    expect(capabilityCenterPaginationMode({ section: "skill", stateFilter: "blocked" })).toBe(
      "server",
    );
    expect(capabilityCenterPaginationMode({ section: "skill", stateFilter: "blocked" })).toBe(
      "server",
  ...
  it("keeps transiently changed rows in a filtered view without duplicating matches", () => {
    ...
    expect(
      mergeCapabilityCenterStickyItems([visible], [visible, justHidden]).map(
        (item) => item.capability_id,
      ),
    ).toEqual(["skill:team:review", "skill:team:hidden"]);
  });

  it("suppresses locally removed rows even when the remote registry still indexes them", () => {
    const visible = row({
      capability_id: "skill:openclaw:afrexai-roofing-contractor",
    ...
    expect(
      filterCapabilityCenterRemovedItems([visible, kept], new Set([visible.capability_id])).map(
        (item) => item.capability_id,
      ),
    ).toEqual([kept.capability_id]);
  });
  ...
  it("protects inactive built-ins and labels row actions by capability type", () => {
    expect(
      capabilityCenterRemovalAction(
        row({ capability_id: "skill:cccc:install", kind: "skill", source_id: "cccc_builtin" }),
      ),
    ).toBe("none");
    expect(
      capabilityCenterRemovalAction(
        row({ capability_id: "pack:diagnostics", kind: "pack", source_id: "cccc_builtin" }),
        { enabled: true },
      ),
    ).toBe("disable");
    expect(
  ...
  it("hides readonly builtin system rows by default but preserves search discoverability", () => {
    const items = [
      ...
      row({
        capability_id: "mcp:github/chrome-devtools-mcp",
        kind: "mcp_toolpack",
        source_id: "mcp_registry_official",
        name: "chrome devtools mcp",
      }),
      row({
        capability_id: "pack:enabled-builtin",
        kind: "pack",
        source_id: "cccc_builtin",
    ...
    expect(
      capabilityCenterFilterItemsForSystemVisibility(items, {
        showSystem: false,
        query: "",
        state: { ...state, enabled: [{ capability_id: "pack:enabled-builtin" }] },
