...
} from "../../../src/pages/chat/voice-secretary/voiceTranscriptSource";
...
describe("voice transcript model", () => {
  ...
  it("filters transcript rows to the selected document path", () => {
    ...
    expect(
      filterVoiceTranscriptItemsForDocument(items, "docs/voice-secretary/second.md").map(
        (item) => item.id,
      ),
    ).toEqual(["second-doc"]);
  ...
  it("does not upsert live transcript rows outside document mode", () => {
    ...
    expect(
      upsertLiveVoiceTranscriptItem([existing], {
        id: "live",
        phase: "interim",
        text: "live text",
        mode: "instruction",
  ...
  it("dedupes restored and local transcript rows for the same document", () => {
    ...
    const restored = makeTranscriptItem({
      id: "restored",
  ...
  it("does not treat live preview rows as displayable final transcript rows", () => {
    expect(
      isDisplayableFinalVoiceTranscriptItem(
        makeTranscriptItem({ id: "voice-stream-live", text: "live preview" }),
      ),
    ).toBe(false);
    expect(
      isDisplayableFinalVoiceTranscriptItem(
        makeTranscriptItem({ id: "final-session-segment-0", text: "final transcript" }),
      ),
    ).toBe(true);
  });
  ...
  it("does not force a single speaker badge on mixed-speaker transcript rows", () => {
    ...
    const items = annotateVoiceTranscriptItemsWithSpeakers(
      [item],
      [
  ...
  it("formats source labels and compact source details", () => {
    ...
    expect(
      ...
      "sherpa_onnx_sense_voice_zh_en_ja_ko_yue_int8 · sense_voice · lang=auto · 2 chunks · fallback=vad_failed",
    );
  });
});
