...
function buildActivityNarrative(
  ...
  const primaryLabel = command
    ...
    : filePaths.length > 0
      ...
      : truncateText(
          structuredLabel || summary || formatStreamingActivityKind(activity.kind),
          density === "expanded" ? 88 : 72,
        );
  ...
  const showToolDetail = Boolean(
    toolLabel && toolLabel !== primaryLabel && toolLabel !== primaryTitle && toolLabel !== summary,
  );
  const detailRows = normalizeMetaLines([
    ...
    cwd ? `cwd\t${cwd}` : undefined,
    filePaths.length > 0
      ? `files\t${filePaths.map((path) => getPathTail(path, 3)).join(", ")}`
      : undefined,
    status ? `state\t${status}` : undefined,
  ])
    .map((line) => {
      const [label, ...rest] = line.split("\t");
      return { label, value: truncateText(rest.join("\t"), density === "expanded" ? 140 : 96) };
    })
...
function getActivityRowClassName(
  density: HeadlessLiveTraceDensity,
  isDark: boolean,
  live: boolean,
): string {
  return classNames(
    ...
    live
      ? isDark
        ? "border-white/12 bg-white/[0.06] shadow-[0_12px_28px_rgba(0,0,0,0.18)]"
        : "border-black/10 bg-white shadow-[0_10px_24px_rgba(15,23,42,0.08)]"
      : isDark
        ? "border-white/8 bg-white/[0.03]"
        : "border-slate-200/90 bg-slate-50/88",
  );
}

function getActivityKindBadgeClassName(
  density: HeadlessLiveTraceDensity,
  isDark: boolean,
  live: boolean,
): string {
  return classNames(
    ...
    live
      ? isDark
        ...
        : "border-black/10 bg-slate-100 text-[rgb(35,36,37)]"
      : isDark
        ...
        >
          {narrative.detailRows.map((row) => (
            <div key={`${row.label}:${row.value}`} className="min-w-0">
              <span
                className={classNames(
                  "mr-1 uppercase tracking-[0.12em]",
...
export function HeadlessLiveTrace({
  ...
  const labels = useMemo<HeadlessLiveTraceLabels>(
    () => ({
      ...
      toolCalls: (count: number) =>
        t("headlessTraceToolCalls", { count, defaultValue: "{{count}} tool calls" }),
  ...
  const timelineGroups = useMemo(
    () => buildHeadlessPreviewRenderGroups(timelineEntries),
    [timelineEntries],
  );
  const sessionCount =
    Array.isArray(previewSessions) && previewSessions.length > 0
      ...
      >
        <div
          className={classNames(
            ...
            isDark ? "border-white/10 bg-white/[0.04]" : "border-black/8 bg-white/88",
          )}
        >
    ...
    >
      <div
        className={classNames(
          ...
          isDark
            ? "border-white/8 bg-[rgba(10,11,14,0.78)]"
            : "border-black/6 bg-[rgba(255,255,255,0.78)]",
        )}
      >
            ...
            <span
              className={classNames(
                ...
                isDark
                  ...
                  : "border-black/8 bg-black/[0.03] text-slate-600",
              )}
            >
              <ClockIcon size={12} />
      ...
      <div
        className={classNames(
          density === "expanded" ? "space-y-4 px-4 py-4 sm:px-5 sm:py-5" : "space-y-3 px-4 py-4",
        )}
      >
        {timelineGroups.map((group, index) => {
          const previousGroup = index > 0 ? timelineGroups[index - 1] : null;
          const showSessionDivider =
            sessionCount > 1 && previousGroup?.pendingEventId !== group.pendingEventId;
          return (
            <div
              key={group.id}
              className={classNames(density === "expanded" ? "space-y-3" : "space-y-2.5")}
            >
              {showSessionDivider ? (
                <div className="flex items-center gap-3 py-1">
                  <span
                    className={classNames("h-px flex-1", isDark ? "bg-white/8" : "bg-slate-200")}
                  />
                  <span
                    className={classNames(
                      ...
                      isDark
                        ...
                        : "border-black/8 bg-black/[0.03] text-slate-500",
                    )}
                  >
              ...
              {group.kind === "message" ? (
                    ...
                    className={classNames(
                      ...
                      group.entry.streamPhase === "final_answer"
                        ? isDark
                          ...
                          : "border-black/10 bg-white shadow-[0_10px_26px_rgba(15,23,42,0.06)]"
                        : isDark
                  ...
                  >
                        ...
                        <span
                          className={classNames(
                            ...
                            isDark
                              ...
                              : "border-black/8 bg-black/[0.03] text-[rgb(35,36,37)]/80",
                          )}
                        >
                          {getTracePhaseLabel(group.entry.streamPhase, labels)}
                        </span>
                    ...
                    {shouldRenderPreviewMarkdown(group.entry.streamPhase) ? (
                      <LazyMarkdownRenderer
                        ...
                        fallback={
                          <div className={getMessageTextClassName(density, isDark)}>
                            {group.entry.text}
                          </div>
                        }
                      />
                ...
                >
                  <div
                    className={classNames(density === "expanded" ? "pt-[0.55rem]" : "pt-[0.4rem]")}
                  >
                    <span
                      className={classNames(
                        "block rounded-full",
                  ...
                  <div className={getActivityBandClassName(density)}>
                    {group.entries.map((entry) => (
                      <ActivityRow key={entry.id} entry={entry} density={density} isDark={isDark} />
                    ))}
                  </div>
                </div>
              )}
