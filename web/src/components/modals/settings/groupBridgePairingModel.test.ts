...
describe("groupBridgePairingModel", () => {
  it("gates invite creation on the current local group only", () => {
    ...
    expect(
      canCreateInvite({ groupId: "g1", busy: false, issuerEndpoint: "https://issuer.example" }),
    ).toBe(true);
  });
  ...
  it("accepts either a raw pairing code or a JSON connection info payload", () => {
    ...
    expect(
      parseConnectionInfoInput(
        JSON.stringify({ pairing_code: "WXYZ-9876", group_id: "g_local", peer_id: "peer_host" }),
      ),
    ).toMatchObject({
  ...
  it("normalizes issuer endpoints the same way as the backend integrity material", async () => {
    ...
    expect(normalizeIssuerEndpoint("https://cccc.example.com/some/path?token=secret#frag")).toBe(
    ...
    const payload = await buildConnectionInfoPayload({
      ...
      digestHex: async (material) => {
        expect(material).toBe(
          "https://cccc.example.com|g_issuer|Issuer Group|peer_issuer|ABCD-1234|2026-06-15T04:00:00Z|pinv_1",
        );
        return "abc123";
      },
    });
  ...
  it("rejects invalid issuer endpoints before creating connection info", () => {
    expect(() => normalizeIssuerEndpoint("ftp://cccc.example.com")).toThrow(
      "issuer_endpoint must be an http(s) URL",
    );
    expect(() => normalizeIssuerEndpoint("https://")).toThrow(
      "issuer_endpoint must be an http(s) URL",
    );
  });
  ...
  it("normalizes copied connection info before sending the pairing code", () => {
    expect(parseConnectionInfoInput('"ABCD-1234"')).toEqual({ pairingCode: "ABCD-1234" });
    expect(
      parseConnectionInfoInput('```json\n{"pairing_code":"WXYZ-9876","group_id":"g_local"}\n```'),
    ).toMatchObject({ pairingCode: "WXYZ-9876", remoteGroupId: "g_local", isRemote: false });
    expect(
      parseConnectionInfoInput(
        '{\n  "type": "cccc.group_bridge_session.connection_info",\n  "pairing_code": " lmno-4567 "\n}',
      ),
    ).toMatchObject({ pairingCode: "LMNO-4567", isRemote: false });
  });

  it("formats peer labels without exposing secrets", () => {
    expect(formatPeerLabel({ remote_peer_id: "peer_remote", remote_group_id: "g_remote" })).toBe(
      "g_remote / peer_remote",
    );
    expect(
      formatPeerLabel({
    ...
    ).toBe("Remote Group / peer_remote");
    expect(formatPeerLabel({ remote_peer_id: "", remote_group_id: "g_remote" })).toBe("g_remote");
    expect(
      formatPeerLabel({ remote_peer_id: "peer_remote", credential_ref: "sec_remote" }),
    ).not.toContain("sec_remote");
    ...
    expect(
      formatRemoteGroupLabel({ remote_peer_id: "peer_remote", remote_group_id: "g_remote" }),
    ).toBe("g_remote");
  ...
  it("shows only the latest sent request per remote issuer", () => {
    const outbounds = [
      ...
      {
        outbound_id: "pout_other",
        issuer_endpoint: "https://other.example",
        issuer_group_id: "g_other",
        issuer_peer_id: "peer_other",
        status: "submitted",
        updated_at: "2026-06-15T02:00:00Z",
      },
      {
        outbound_id: "pout_new",
        issuer_endpoint: "https://issuer.example",
        issuer_group_id: "g_issuer",
        issuer_peer_id: "peer_issuer",
        status: "submitted",
    ...
    expect(projectRecentOutbounds(outbounds).map((outbound) => outbound.outbound_id)).toEqual([
      "pout_new",
      "pout_other",
    ]);
  });
  ...
  it("hides approved sent requests from the recent outbound projection", () => {
    const outbounds = [
      ...
      },
      {
        ...
        issuer_endpoint: "https://failed.example",
        issuer_group_id: "g_failed",
        issuer_peer_id: "peer_failed",
        status: "failed",
        updated_at: "2026-06-15T02:00:00Z",
      },
      {
        outbound_id: "pout_rejected",
        issuer_endpoint: "https://rejected.example",
        issuer_group_id: "g_rejected",
        issuer_peer_id: "peer_rejected",
        status: "rejected",
        updated_at: "2026-06-15T01:00:00Z",
      },
    ] as GroupBridgePairingOutbound[];
  ...
  it("continues polling remote status for submitted and pending outbound requests", () => {
    ...
    expect(projectSyncableOutbounds(outbounds).map((outbound) => outbound.outbound_id)).toEqual([
      "pout_submitted",
      "pout_pending",
    ]);
  });
  ...
  it("projects overview chips from identity and pairing lists", () => {
    ...
    expect(
      projectPairingOverview({
        identity: { node_id: "node1", peer_id: "peer1" },
        requests,
        trusts,
      }),
    ).toEqual({ identityReady: true, pendingCount: 1, trustedCount: 1 });
    expect(projectPairingOverview({ identity: null, requests: [], trusts: [] }).identityReady).toBe(
      false,
    );
  });
  ...
  it("maps backend issuer endpoint policy errors to user-facing locale keys", () => {
    expect(userFacingPairingErrorKey("unsafe issuer_endpoint is not allowed")).toBe(
      "group_bridge.unsafeIssuerEndpointBlocked",
    );
