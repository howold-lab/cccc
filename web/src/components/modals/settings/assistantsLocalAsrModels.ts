...
function withDefaultStreamingConfig(
  ...
  return {
    ...
    title: String(
      model?.title || "sherpa-onnx streaming Paraformer trilingual zh/cantonese/en int8",
    ),
...
export function resolveLocalAsrModels(args: {
  ...
  const configuredModel = args.configuredModelId
    ? args.serviceModelsById[args.configuredModelId]
    : undefined;
  ...
  const serviceAsrModels = args.serviceModels.filter(
    (model) => String(model.kind || "").trim() === "asr",
  );
